...
export default function ClientRegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      await api.post('/auth/client/register', { name, email, password });
  ...
  return (
    <div className="flex min-h-screen items-center justify-center">
      <div className="w-full max-w-sm rounded-xl bg-white/10 p-8 backdrop-blur-lg">
        <h1 className="mb-6 text-2xl font-bold">{t('client.register')}</h1>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="name" className="text-sm text-gray-400">{t('client.name')}</label>
            <input
              id="name"
              type="text"
              required
          ...
          >
            {loading ? t('auth.pleaseWait') : t('client.createAccount')}
          </button>
        </form>
