...
export default function ShortCodeProposalPage() {
  ...
  useEffect(() => {
    api.get<Proposal>(`/proposals/code/${shortCode}`)
      .then(setProposal)
      .catch((err) => {
  ...
  // Initialize item statuses from proposal data
  if (proposal && Object.keys(itemStatuses).length === 0 && proposal.items.length > 0) {
  ...
  const handleSubmitResponse = useCallback(async () => {
    ...
    try {
      ...
      const updated = await api.patch<Proposal>(`/proposals/token/${proposal.token}/respond`, { items });
      setProposal(updated);
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-gray-950">
      <div className="mx-auto max-w-3xl p-8">
        ...
        {!isExpired && !isTerminal && !isRespuestaParcial && (
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="rounded-md bg-white/10 px-4 py-1.5 text-sm font-medium text-white/70 transition hover:bg-white/20"
            >
              {copied ? t('shortCode.copied') : t('shortCode.copyUrl')}
            </button>
          </div>
        </div>
