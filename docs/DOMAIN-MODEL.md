...
interface Menu {
  ...
  isActive: boolean;               // Default false, toggled on publish
...
- `isActive` defaults to `false`; toggled via publish endpoint
...
```typescript
interface MenuItem {
  _id: ObjectId;                   // Auto-generated by Mongoose subdocument
